...
def compute_scores_cnn(positions_dict, feature, a_len, predict, method='SampleShapley'):
    """
    ...
    real_ids = feature['input_x'][0, 0:a_len]
    inputs = np.array(real_ids) * positions
    #print('inputs shape is:', inputs.shape)

    batch_in = {'input_x':[], 'input_y':[]}
    for j in range(inputs.shape[0]):
        mask_input = np.concatenate((inputs[j], feature['input_x'][0, a_len:]))
        batch_in['input_x'].append(mask_input)
        batch_in['input_y'].append(feature['input_y'])

    batch_dict = {
        'input_x': np.array(batch_in['input_x']),
        'input_y': np.array(batch_in['input_y']),
    }
    x = {
        'input_x': np.array([feature['input_x']]),
        'input_y': np.array([feature['input_y']]),
    }
...
def compute_scores_seperate_cnn(positions_dict, feature, a_len, predict, method='SampleShapley'):
    """
    ...
    real_ids = feature['input_x'][0:a_len]
    inputs = np.array(real_ids) * positions
    #print('inputs shape is:', inputs.shape)

    batch_in = {'input_x': [], 'input_y': []}
    for j in range(inputs.shape[0]):
        mask_input = np.concatenate((inputs[j], feature['input_x'][a_len:]))
        batch_in['input_x'].append(mask_input)
        batch_in['input_y'].append(feature['input_y'])
